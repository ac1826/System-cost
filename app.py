...
def _resolve_tsc_key_columns(df):
    ...
    for i in range(min(len(df), 8)):
        ...
        if mat_key not in row:
            continue
        mat_col = row.index(mat_key)
        if label_key in row:
            label_col = row.index(label_key)
            impact_col = row.index(impact_key) if impact_key in row else None
            return {'key_row': i, 'mat_col': mat_col, 'label_col': label_col, 'impact_col': impact_col}
        if spec_key in row:
            spec_col = row.index(spec_key)
...
def _resolve_tsc_raw_layout(df):
    ...
    for i in range(min(len(df), 8)):
        row = [_clean_colname(v) for v in df.iloc[i].tolist()]
        digit_cols = [c for c, s in enumerate(row) if s.isdigit()]
        if digit_cols and (metric_key in row or comp_key in row):
            raw_row = i
            break
    if raw_row is None:
        comp_row = _find_header_row(df, comp_key)
        if comp_row is not None and comp_row > 0:
            raw_row = comp_row - 1
    spec_row = raw_row + 1 if raw_row is not None and raw_row + 1 < len(df) else None
    comp_col = None
    for r in [raw_row, spec_row]:
        if r is None:
            continue
        row = [_clean_colname(v) for v in df.iloc[r].tolist()]
...
def _scrap_factor_for_category(category, material_desc='', material_no='', has_negative=True, other_factor=None):
...
def _find_tsc_metrics(tsc_file, sheet_name, material_no, row_label):
    try:
        ...
        return None
    metric_key = '修形前原料综合耗用单价'
    metric_row = _find_header_row(df, metric_key)
    if metric_row is None:
        return None
    header = [_clean_colname(c) for c in df.iloc[metric_row].astype(str).tolist()]
    header_idx = {}
    ...
    mat_col = key_cols.get('mat_col', 2)
    label_col = key_cols.get('label_col', 4)
    start_row = max(metric_row + 1, key_cols.get('key_row', 0) + 1, (spec_row + 1) if spec_row is not None else 0)
    for i in range(start_row, len(df)):
        mat = _normalize_mat(df.iat[i, mat_col]) if mat_col < df.shape[1] else ''
        label = str(df.iat[i, label_col]).strip() if label_col < df.shape[1] else ''
        if mat == material_no and _match_tsc_row_label(label, row_label):
...
def _get_tsc_raw_columns(tsc_file, sheet_name):
    ...
    for c in range(df.shape[1]):
        s = _clean_colname(df.iat[raw_row, c])
        if s.isdigit():
            raw_cols.append((c, s))
...
def compute(compare_file, rawlist_file, q3_file, map_file, sheet_name=None, month_label='11月', quarter_label='Q3', month_code=''):
    ...
    for mat, v in agg.items():
        ...
        factor = _scrap_factor_for_category(v['分类'], v.get('物料描述', ''), v.get('物料号', ''), v.get('存在负值实际数量', False), v.get('其他系数'))
        raw_cost = None
    ...
    for mat, v in agg.items():
        ...
        q3_ratio = _find_tsc_value(q3_file, tsc_sheet, mat, reference_ratio_label)
        if q3_ratio is None and fallback_ratio_label != reference_ratio_label:
            q3_ratio = _find_tsc_value(q3_file, tsc_sheet, mat, fallback_ratio_label)
        nov_ratio = None
        q3_metrics = _find_tsc_metrics(q3_file, tsc_sheet, mat, reference_price_label)
        if q3_metrics is None and fallback_price_label != reference_price_label:
            q3_metrics = _find_tsc_metrics(q3_file, tsc_sheet, mat, fallback_price_label)
    ...
    def build_matrix_rows(materials, raw_cols, comp_col, df_q3, category_label):
        ...
        for mat in materials:
            ...
            q3_price = _find_tsc_row_values(df_q3, mat, reference_price_label, raw_cols, comp_col)
            if not q3_price and fallback_price_label != reference_price_label:
                q3_price = _find_tsc_row_values(df_q3, mat, fallback_price_label, raw_cols, comp_col)
            q3_ratio = _find_tsc_row_values(df_q3, mat, reference_ratio_label, raw_cols, comp_col)
            if not q3_ratio and fallback_ratio_label != reference_ratio_label:
                q3_ratio = _find_tsc_row_values(df_q3, mat, fallback_ratio_label, raw_cols, comp_col)

            for label, source in [
                (f'{month_label}实际单价', raw_unit),
                (reference_price_label, q3_price),
    ...
    if not raw_cols_other:
        raw_cols_other, spec_map_other = _build_fallback_raw_columns(other['物料号'].unique())
    raw_usage_other = build_matrix_rows(other['物料号'].unique(), raw_cols_other, comp_col_other, df_q3_other, '其他')
...
def to_excel_bytes(
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        def _remove_thousand_sep_for_xhyl(ws):
            ...
            if not target_col:
                return
            start_row = 5 if ws.title in ('腿肉TSC', '胸肉TSC', '其他TSC') else 2
            for r in range(start_row, ws.max_row + 1):
        ...
        for sheet_name in [f'{prefix}腿肉', f'{prefix}胸肉', f'{prefix}其他', '腿肉TSC', '胸肉TSC', '其他TSC']:
            if sheet_name in writer.book.sheetnames:
        ...
        for ws in writer.book.worksheets:
            if ws.title in ('腿肉TSC', '胸肉TSC', '其他TSC'):
